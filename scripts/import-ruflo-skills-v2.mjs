...
async function resolveSourceTree() {
  const commitUrl = `${API_BASE}/repos/${SOURCE_OWNER}/${SOURCE_REPO}/commits/${encodePathSegments(sourceRef)}`;
...
function canonicalSkillName(root, rawName) {
  const base = slugifySkillName(root || rawName || 'ruflo-skill') || 'ruflo-skill';
  if (base.length <= MAX_SKILL_NAME_LENGTH) {
    return base;
  }
  const suffix = createHash('sha256').update(root || rawName || base).digest('hex').slice(0, 8);
  return `${base.slice(0, MAX_SKILL_NAME_LENGTH - suffix.length - 1).replace(/-+$/g, '')}-${suffix}`;
}

function normalizeSkillMarkdown(markdown, sourcePath, root) {
  ...
  try {
    ...
    throw new Error(`Invalid YAML frontmatter in ${sourcePath}: ${error.message}`);
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error(`Invalid YAML frontmatter object in ${sourcePath}`);
  }

  const entries = Object.entries(parsed);
  const rawNameEntry = entries.find(([key]) => key.toLowerCase() === 'name');
  const descriptionEntry = entries.find(([key]) => key.toLowerCase() === 'description');
  const whenToUseEntry = entries.find(([key]) => key.toLowerCase() === 'when-to-use');
  const rawName = typeof rawNameEntry?.[1] === 'string' ? rawNameEntry[1].trim() : '';
  const rawDescription = descriptionEntry?.[1] ?? whenToUseEntry?.[1];
  const description =
    typeof rawDescription === 'string'
      ? rawDescription.trim().slice(0, MAX_SKILL_DESCRIPTION_LENGTH)
      : '';

  if (!description) {
    throw new Error(`Missing skill description in ${sourcePath}`);
  }

  const name = canonicalSkillName(root, rawName);
  const rest = Object.fromEntries(
    entries.filter(([key]) => !['name', 'description'].includes(key.toLowerCase())),
  );
  const frontmatter = {
    name,
    description,
  ...
  try {
    ...
    for (const record of downloaded) {
      ...
      if (record.entry.path === 'SKILL.md' || record.entry.path.endsWith('/SKILL.md')) {
        ...
        const previous = names.get(normalizedSkill.name);
        if (previous) {
          throw new Error(
            `Duplicate normalized skill name "${normalizedSkill.name}" in ${previous} and ${record.entry.path}`,
          );
        }
        names.set(normalizedSkill.name, record.entry.path);
        await writeFile(destination, normalizedSkill.content, { mode: 0o644 });
      } else {
        await writeFile(destination, record.buffer, { mode: 0o644 });
      }
    }

    if (names.size !== skillRoots.length) {
      throw new Error(`Expected ${skillRoots.length} skills but normalized ${names.size}`);
    }
    ...
    console.log(
      `[ruflo-skills] Imported ${skillRoots.length} skills (${files.length} files, ${totalBytes} bytes) from ${resolvedCommit}; manifest ${manifestStats.size} bytes`,
    );
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
