...
function requestHeaders({ json = false } = {}) {
  const headers = {
    'User-Agent': 'CyberTools-Ruflo-Skills-Importer',
  };
...
function parseSkillFrontmatter(markdown, sourcePath) {
  ...
  if (!name || !/^[a-z0-9][a-z0-9-]*$/.test(name)) {
    throw new Error(`Invalid or missing skill name in ${sourcePath}`);
  }
  if (!description) {
...
async function main() {
  ...
  try {
    const downloaded = await mapWithConcurrency(files, DOWNLOAD_CONCURRENCY, async (entry) => {
      ...
      const destination = path.join(tempRoot, ...entry.path.split('/'));
    ...
    for (const root of skillRoots) {
      ...
      const record = downloaded.find(({ entry }) => entry.path === skillPath);
      if (!record) {
        throw new Error(`Downloaded package is missing ${skillPath}`);
      }

      const metadata = parseSkillFrontmatter(record.buffer.toString('utf8'), skillPath);
      const previous = names.get(metadata.name);
      if (previous) {
        throw new Error(`Duplicate skill name "${metadata.name}" in ${previous} and ${skillPath}`);
      }
